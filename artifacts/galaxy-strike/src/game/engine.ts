...
import { worldFromWave, getWorld, WorldDef, WORLDS } from './worlds';
...
const TOTAL_WAVES = TOTAL_WORLDS * WAVES_PER_WORLD; // 50
...
export class GameEngine {
  ...
  currentWorld: WorldDef = WORLDS[0];
  ...
  // World-clear cinematic pause
  worldClearPending: boolean = false;
  worldClearCountdown: number = 0; // seconds to wait before starting next world
  ...
  onWorldClear: ((worldNum: number, world: WorldDef) => void) | null = null;
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.player = new Player(canvas.width / 2, canvas.height - 120, fireRate, damage, maxHp, this.saveData.selectedShip);
    ...
    this.input.onMoveCallback = (dx, dy) => {
      ...
      void speed; // used for future analog stick scaling
    ...
    this.wave = 1;
    this.worldNumber = 1;
    this.currentWorld = getWorld(1);
    this.renderer.setWorld(1);
    this.startWave();
  }
  ...
  togglePause() {
    ...
    if (!this.isPaused) { this.lastTime = performance.now(); this.loop(this.lastTime); }
  }
  ...
  startWave() {
    ...
    // Update world
    this.worldNumber = worldFromWave(this.wave);
    this.currentWorld = getWorld(this.worldNumber);
    this.renderer.setWorld(this.worldNumber);
  }
  ...
  spawnExplosion(x: number, y: number, isLarge: boolean) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.particles.push(new Particle(x, y, Math.cos(angle) * speed, Math.sin(angle) * speed, Math.random() * 0.5 + 0.2, colors[Math.floor(Math.random() * colors.length)], Math.random() * 3 + 1));
    }
  ...
  private fireWeapon() {
    ...
    if (lvl >= 2) {
      const s1 = new Bullet(px - 20, py, -780, dmg * 0.7, false); s1.vx = -120; this.bullets.push(s1);
      const s2 = new Bullet(px + 20, py, -780, dmg * 0.7, false); s2.vx =  120; this.bullets.push(s2);
    }
    if (lvl >= 3 && Math.floor(now / 300) % 3 === 0) {
      const ml = new Bullet(px - 26, py, -400, dmg * 1.5, false, 8, 12); ml.isMissile = true; this.bullets.push(ml);
      const mr = new Bullet(px + 26, py, -400, dmg * 1.5, false, 8, 12); mr.isMissile = true; this.bullets.push(mr);
    }
    if (lvl >= 4) {
      const q1 = new Bullet(px - 28, py - 10, -800, dmg * 0.6, false); q1.vx = -80; this.bullets.push(q1);
      const q2 = new Bullet(px + 28, py - 10, -800, dmg * 0.6, false); q2.vx =  80; this.bullets.push(q2);
    }
  ...
  update(dt: number) {
    // While world-clear cinematic is running, freeze gameplay
    if (this.worldClearPending) {
      this.worldClearCountdown -= dt;
      if (this.worldClearCountdown <= 0) {
    ...
    this.enemies.forEach(e => {
      ...
      if (performance.now() - e.lastFired > e.fireRate) {
        ...
        if (e.type === 'drifter' && Math.random() > 0.5) {
          this.bullets.push(new Bullet(cx, cy, 300, 10, true)); e.lastFired = performance.now();
        } else if (e.type === 'weaver' && Math.random() > 0.7) {
          this.bullets.push(new Bullet(cx, cy, 320, 8, true)); e.lastFired = performance.now();
        } else if (e.type === 'tank') {
          this.bullets.push(new Bullet(cx - 10, cy, 250, 15, true, 6, 10));
          this.bullets.push(new Bullet(cx + 10, cy, 250, 15, true, 6, 10));
          e.lastFired = performance.now();
        } else if (e.type === 'bomber' && Math.random() > 0.6) {
          for (let a = 0; a < Math.PI * 2; a += Math.PI / 3) {
            const b = new Bullet(cx, cy, 240, 12, true);
            b.vx = Math.sin(a) * 240; b.vy = Math.cos(a) * 240;
            this.bullets.push(b);
          }
          e.lastFired = performance.now();
        } else if (e.type.startsWith('boss')) {
          this.bullets.push(new Bullet(cx - 20, cy, 400, 20, true));
          this.bullets.push(new Bullet(cx,       cy, 450, 20, true));
          this.bullets.push(new Bullet(cx + 20,  cy, 400, 20, true));
          // Phase 2 adds angled shots
          if (e.phase === 1) {
            const bL = new Bullet(cx, cy, 380, 18, true); bL.vx = -180; this.bullets.push(bL);
            const bR = new Bullet(cx, cy, 380, 18, true); bR.vx =  180; this.bullets.push(bR);
          }
          e.lastFired = performance.now();
        }
      }
    ...
    this.bullets.forEach(b => {
      if ((b as any).isMissile && !b.isEnemy && b.active) {
        let closest: Enemy | null = null, closestDist = 99999;
        ...
        if (closest) {
          ...
          if (dist > 0) {
            ...
            if (speed > 600) { const sc = 600 / speed; if (b.vx !== undefined) b.vx *= sc; b.vy *= sc; }
          }
        }
      }
      b.update(dt);
    ...
    this.collectibles.forEach(c => {
      ...
      if (dist < magnetDist) { c.x += (dx / dist) * 300 * dt; c.y += (dy / dist) * 300 * dt; }
    });
    this.particles.forEach(p => p.update(dt));
    ...
    for (const b of this.bullets) {
      ...
      for (const e of this.enemies) {
        ...
        if (b.collidesWith(e)) {
          ...
          if (!e.active) {
            this.killCount++;
            this.score += e.type.startsWith('boss') ? 2000 + this.worldNumber * 500 : 50 + this.worldNumber * 10;
    ...
    for (const b of this.bullets) {
      ...
      if (b.collidesWith(this.player)) {
        ...
        this.shakeAmount = 5; audio.playPlayerHit();
    ...
    for (const e of this.enemies) {
      ...
      if (this.player.collidesWith(e)) {
        this.player.takeDamage(20); this.shakeAmount = 10;
        e.takeDamage(1000); if (!e.active) this.spawnExplosion(e.x, e.y, false);
        audio.playPlayerHit();
        if (this.player.hp <= 0) { this.gameOver(); return; }
      }
    }
    ...
    for (const c of this.collectibles) {
      ...
      if (this.player.collidesWith(c)) {
        ...
        if (c.type === 'coin') {
          ...
          this.coinsCollected += amount; this.score += 10; audio.playCoin();
        } else if (c.type === 'gem') {
          this.gemsCollected++; audio.playCoin();
        } else if (c.type === 'shield') {
          this.player.hp = Math.min(this.player.maxHp, this.player.hp + this.player.maxHp * 0.25); audio.playPowerUp();
        } else if (c.type === 'rapidfire') {
          this.player.powerUps.rapidfire = 5.0; audio.playPowerUp();
        } else if (c.type === 'nuke') {
          this.enemies.forEach(en => en.takeDamage(9999));
          this.shakeAmount = 20; audio.playExplosion(true);
        }
      }
    }
    ...
    if (this.waveActive && this.enemiesToSpawn <= 0 && this.enemies.length === 0) {
      ...
      if (!this.waveClearNotified) {
        ...
        // Check world clear (boss wave = wave 5, 10, 15, ...)
        const waveInWorld = ((this.wave - 1) % 5) + 1;
        if (waveInWorld === 5) {
          const clearedWorld = worldFromWave(this.wave);

          // Victory condition: all 10 worlds done
          if (clearedWorld >= TOTAL_WORLDS && this.wave >= TOTAL_WAVES) {
            // Give extra coin/gem reward then trigger victory after delay
            this.coinsCollected += 5000;
            this.gemsCollected  += 100;
            setTimeout(() => {
              this.stop();
            ...
            // World cleared — trigger cinematic pause
            this.worldClearPending = true;
            this.worldClearCountdown = 5.0; // 5s for React overlay to show
            if (this.onWorldClear) this.onWorldClear(clearedWorld, this.currentWorld);
          }
        }
      }
    }

    if (!this.waveActive && !this.worldClearPending) {
  ...
  gameOver() {
    ...
    if (this.wave  > save.highWave)  saveGame({ highWave:  this.wave  });
    if (this.onGameOver) {
      this.onGameOver({ score: this.score, wave: this.wave, coins: this.coinsCollected, gems: this.gemsCollected, kills: this.killCount });
    }
  }
}
