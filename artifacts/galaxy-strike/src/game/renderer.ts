...
export class Renderer {
  ...
  drawPlayer(player: Player) {
    ...
    // Engine flame (bottom center)
    const flameFlicker = 0.8 + Math.sin(this.time * 18) * 0.2;
    const flameGrad = ctx.createRadialGradient(0, h * 0.35, 0, 0, h * 0.35, w * 0.55 * flameFlicker);
    ...
    ctx.beginPath();
    ctx.ellipse(0, h * 0.42, w * 0.28 * flameFlicker, h * 0.22 * flameFlicker, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    // Nose tip
    ctx.beginPath();
    ctx.arc(0, -h * 0.44, w * 0.07, 0, Math.PI * 2);
    ...
    // Engine nozzle
    ctx.beginPath();
    ctx.ellipse(0, h * 0.38, w * 0.16, h * 0.07, 0, 0, Math.PI * 2);
    ctx.fillStyle = '#333355';
    ctx.fill();
    this._outline(ctx, '#000', 2);

    // Outer neon glow ring
    this._glow(ctx, '#00f7ff', 20);
  ...
  drawEnemy(enemy: Enemy) {
    ...
    switch (enemy.type) {
      ...
      default:
        if (enemy.type.startsWith('boss')) this._drawBoss(ctx, enemy);
        else this._drawDrifter(ctx, enemy);
    }
  ...
  private _drawDrifter(ctx: CanvasRenderingContext2D, e: Enemy) {
    ...
    ctx.beginPath();
    ctx.ellipse(0, h * 0.1, w * 0.5, h * 0.28, 0, 0, Math.PI * 2);
    ...
    // Dome highlight
    ctx.beginPath();
    ctx.ellipse(-w * 0.06, -h * 0.1, w * 0.09, h * 0.07, -0.4, 0, Math.PI * 2);
    ...
    for (let i = -1; i <= 1; i++) {
      ctx.beginPath();
      ctx.arc(i * w * 0.18, h * 0.26, 3, 0, Math.PI * 2);
  ...
  private _drawWeaver(ctx: CanvasRenderingContext2D, e: Enemy) {
    ...
    // Central body
    ctx.beginPath();
    ctx.ellipse(0, h * 0.05, w * 0.22, h * 0.38, 0, 0, Math.PI * 2);
    ...
    ctx.fill();
    ctx.beginPath();
    ctx.arc(1, -h * 0.05 + 1, 3, 0, Math.PI * 2);
    ctx.fillStyle = '#000';
    ctx.fill();
  ...
  private _drawTank(ctx: CanvasRenderingContext2D, e: Enemy) {
    ...
    for (const ex of [-w * 0.14, w * 0.14]) {
      ctx.beginPath();
      ctx.arc(ex, -h * 0.06, 5, 0, Math.PI * 2);
      ctx.fillStyle = '#ff0000';
      ctx.fill();
      ctx.beginPath();
      ctx.arc(ex + 1, -h * 0.06 + 1, 2.5, 0, Math.PI * 2);
      ctx.fillStyle = '#000';
      ctx.fill();
      // Angry brow
      ctx.beginPath();
      ctx.moveTo(ex - 5, -h * 0.14);
      ctx.lineTo(ex + (ex < 0 ? 5 : -5), -h * 0.1);
      ctx.lineWidth = 2;
      ctx.strokeStyle = '#000';
      ctx.stroke();
    }
  ...
  private _drawBomber(ctx: CanvasRenderingContext2D, e: Enemy) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      ctx.fill();
      this._outline(ctx, '#000', 1.5);
    }

    // Skull face
    ctx.beginPath();
    ctx.arc(0, -h * 0.04, w * 0.26, 0, Math.PI * 2);
    ...
    for (const ex of [-w * 0.1, w * 0.1]) {
      ctx.beginPath();
      ctx.arc(ex, -h * 0.1, 5, 0, Math.PI * 2);
      ctx.fillStyle = '#000';
      ctx.fill();
    }
  ...
  private _drawSwarm(ctx: CanvasRenderingContext2D, e: Enemy) {
    ...
    // Big cartoon eyes (they're tiny so just one eye)
    ctx.beginPath();
  ...
  private _drawPhantom(ctx: CanvasRenderingContext2D, e: Enemy) {
    const w = e.width, h = e.height;
    const alpha = 0.35 + (Math.sin(e.timeAlive * 5) + 1) / 2 * 0.5;
    ...
    ctx.bezierCurveTo(-w * 0.44, -h * 0.45, w * 0.44, -h * 0.45, w * 0.44, h * 0.1);
    // Wavy bottom
    ctx.lineTo(w * 0.44, h * 0.35);
    ctx.bezierCurveTo(w * 0.3, h * 0.22, w * 0.15, h * 0.48, 0, h * 0.35);
    ctx.bezierCurveTo(-w * 0.15, h * 0.22, -w * 0.3, h * 0.48, -w * 0.44, h * 0.35);
    ctx.closePath();
    ctx.fillStyle = '#aaddff';
    ctx.fill();
    this._outline(ctx, '#003355', 2);

    // Hollow eyes
    for (const ex of [-w * 0.15, w * 0.15]) {
      ctx.beginPath();
      ctx.arc(ex, -h * 0.1, 5, 0, Math.PI * 2);
  ...
  private _drawBoss(ctx: CanvasRenderingContext2D, e: Enemy) {
    const w = e.width, h = e.height;
    const isPhase2 = e.phase === 1;
    const color = isPhase2 ? CONFIG.COLORS.PURPLE : CONFIG.COLORS.RED;

    this._glow(ctx, color, 20);

    // Main body
    ctx.beginPath();
    ctx.roundRect(-w * 0.48, -h * 0.45, w * 0.96, h * 0.9, 12);
    ctx.fillStyle = isPhase2 ? '#330044' : '#220011';
    ctx.fill();
    this._outline(ctx, '#000', 4);

    // Armor panels
    ctx.beginPath();
    ctx.roundRect(-w * 0.35, -h * 0.32, w * 0.7, h * 0.64, 8);
    ctx.fillStyle = isPhase2 ? '#660088' : '#660022';
    ctx.fill();
    this._outline(ctx, '#000', 2);

    // Cannon barrel (bottom center)
    ctx.beginPath();
    ctx.roundRect(-8, h * 0.35, 16, h * 0.18, 4);
    ctx.fillStyle = '#333';
    ctx.fill();
    this._outline(ctx, '#000', 2);
    ...
    for (const sx of [-w * 0.32, w * 0.32]) {
      ctx.beginPath();
      ctx.roundRect(sx - 6, h * 0.28, 12, h * 0.22, 3);
      ctx.fillStyle = '#444';
      ctx.fill();
      this._outline(ctx, '#000', 2);
    }
    ...
    for (let i = 0; i < 3; i++) {
      const ex = (i - 1) * w * 0.22;
      ctx.beginPath();
      ctx.arc(ex, -h * 0.08, 10, 0, Math.PI * 2);
      ctx.fillStyle = isPhase2 ? '#dd00ff' : '#ff0033';
      ctx.fill();
      this._outline(ctx, '#000', 2);
      // pupil glow
      this._glow(ctx, color, 15);
      ctx.beginPath();
      ctx.arc(ex, -h * 0.08, 5, 0, Math.PI * 2);
      ctx.fillStyle = '#fff';
      ctx.fill();
      this._clearGlow(ctx);
    }
    ...
    const barW = w * 0.8;
    ctx.fillStyle = '#333';
    ctx.fillRect(-barW / 2, -h * 0.5 - 12, barW, 7);
    ctx.fillStyle = isPhase2 ? '#dd00ff' : '#ff0033';
    ctx.fillRect(-barW / 2, -h * 0.5 - 12, barW * hpPct, 7);
    this._outline(ctx, '#000', 1.5);
  ...
  drawBullet(b: Bullet) {
    ...
    if (b.isEnemy) {
      ...
      // Enemy bullet: red oval with tail
      ctx.fillStyle = '#ff3366';
      ctx.beginPath();
      ctx.ellipse(b.x, b.y, b.width * 0.8, b.height * 0.4, 0, 0, Math.PI * 2);
      ctx.fill();
    } else {
      ctx.shadowBlur = 12;
      ctx.shadowColor = CONFIG.COLORS.CYAN;
      // Player bullet: cyan laser bolt
      const grad = ctx.createLinearGradient(b.x, b.y - b.height / 2, b.x, b.y + b.height / 2);
      grad.addColorStop(0, 'rgba(0,247,255,0)');
      grad.addColorStop(0.3, '#00f7ff');
      grad.addColorStop(0.7, '#ffffff');
      grad.addColorStop(1, 'rgba(255,255,255,0)');
      ctx.fillStyle = grad;
      ctx.beginPath();
      ctx.ellipse(b.x, b.y, b.width * 0.7, b.height * 0.45, 0, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  drawCollectible(c: Collectible) {
    ...
    const bob = Math.sin(this.time * 4 + c.x) * 2;
    ctx.translate(0, bob);

    if (c.type === 'coin') {
      this._glow(ctx, CONFIG.COLORS.GOLD, 15);
      ctx.beginPath();
      ctx.arc(0, 0, c.width / 2, 0, Math.PI * 2);
      ctx.fillStyle = '#ffd700';
      ctx.fill();
      this._outline(ctx, '#cc8800', 2);
      // $ sign
      ctx.fillStyle = '#cc8800';
      ctx.font = `bold ${c.width * 0.6}px Arial`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('$', 0, 1);
    } else if (c.type === 'gem') {
      this._glow(ctx, CONFIG.COLORS.CYAN, 15);
      ctx.beginPath();
      ...
      ctx.fill();
      this._outline(ctx, '#006688', 2);
    } else if (c.type === 'shield') {
      this._glow(ctx, '#4488ff', 12);
      ctx.beginPath();
      ctx.arc(0, 0, c.width / 2, 0, Math.PI * 2);
      ...
      ctx.font = `bold ${c.width * 0.7}px Arial`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('🛡', 0, 1);
      ...
      ctx.font = `bold ${c.width * 0.65}px Arial`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('💥', 0, 1);
    }
