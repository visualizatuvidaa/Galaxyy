...
export class Player extends Entity {
  ...
  constructor(x: number, y: number, fireRate: number, damage: number, hp: number, shipType: string) {
    super(x, y, 30, 40, hp); // standard player bounds
  ...
  takeDamage(amt: number) {
    ...
    const actualDamage = amt * (1 - this.shieldMod);
...
export class Bullet extends Entity {
  ...
  update(dt: number) {
    this.y += this.vy * dt;
    if (this.y < -50 || this.y > 1000) this.active = false;
  }
}
...
export class Enemy extends Entity {
  ...
  update(dt: number, time: number) {
    ...
    if (this.type === 'weaver') {
      ...
      // Moves diagonally towards player (handled by engine mostly, but base movement here)
      this.y += this.vy * dt;
      this.x += this.vx * dt;
      ...
      this.y += this.vy * 1.5 * dt; // Swarm is faster
      this.x += Math.sin(this.timeAlive * 10) * 20 * dt;
    } else if (this.type === 'boss_dreadnought') {
      if (this.y < 150) this.y += 50 * dt; // enter screen
      else {
        this.x += Math.sin(this.timeAlive) * 50 * dt;
      }
      if (this.hp < this.maxHp * 0.5) this.phase = 1;
    } else if (this.type === 'boss_sentinel') {
      if (this.y < 200) this.y += 50 * dt;
      else {
        this.x = 200 + Math.cos(this.timeAlive) * 80;
        this.y = 200 + Math.sin(this.timeAlive) * 40;
      }
    } else {
      this.y += this.vy * dt;
      this.x += this.vx * dt;
